...
import { getVisibleZeroACemQuestions } from "@/lib/forms/zero-a-cem";
...
const HOTMART_CHECKOUT_URL = ""; // TODO: adicionar link real do checkout 0 a 100
const SUPPORT_WHATSAPP_URL =
  "https://wa.me/5592999999999?text=Oi%2C%20tenho%20d%C3%BAvidas%20sobre%20a%20Mentoria%200%20a%20100";
...
function getCurrentStage(questionKey?: string) {
  ...
  if (finalStageKeys.includes(questionKey)) return 3;

  return 1;
}

export default function ZeroACemPage() {
  ...
  const visibleQuestions = useMemo(() => getVisibleZeroACemQuestions(answers), [answers]);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setSession({
        id: "local-test-session-z100",
        form_slug: "zero-a-cem",
        current_step: 0,
        status: "in_progress",
      });
 ...
 function isQuestionValid(nextValue = value) {
  ...
  async function handleSubmit(customValue?: string | string[]) {
    ...
    const updatedVisibleQuestions = getVisibleZeroACemQuestions(nextAnswers);
  ...
  if (finalScreen === "thinking") {
    return (
      <main className="form-noise-bg relative flex min-h-screen items-center justify-center px-4 py-10 sm:px-6">
        <div className="relative z-10 w-full max-w-3xl">
          <FormStatusScreen
            title="Fechado. Vamos clarear isso agora."
            description="Sua resposta chegou aqui. Minha equipe vai falar com você pra tirar as dúvidas e te deixar seguro antes da matrícula. Você não precisa decidir no escuro. Se preferir, já pode falar com a gente agora no WhatsApp."
  ...
  if (finalScreen === "not_now_end") {
    return (
      <main className="form-noise-bg relative flex min-h-screen items-center justify-center px-4 py-10 sm:px-6">
        <div className="relative z-10 w-full max-w-3xl">
          <FormStatusScreen
            title="Tudo certo. A porta continua aberta."
            description="Agradecemos pelo seu interesse. Ficamos à disposição para atender qualquer dúvida que você tenha. Esperamos que, quando for a hora certa, você comece sua jornada de crescimento com direção, plano e execução junto com a gente."
